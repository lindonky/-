...
- 助力、主动、阻抗三种控制模式
- 三段式 PD 姿态稳定
- 闩锁式急停、IMU 超时、超角度保护和角度/加速度双路跌倒急停
...
完整抬起—回位。身高/小腿长仅用于计算每步等效弦长；页面把各完整步累计后显示为“行走距离”，不代表实测地面位移。
...
训练状态响应包含 `algo`、`device_boot`、`last_score_pct`、`distance_cm_est` 和当前 `limits`，便于导出后追溯算法版本、区分重启后的会话 ID，并保留当次工程阈值。`qualified_pct` 在 MVP 0.3 中表示所有已完成步的连续轨迹评分均值；逐项二值质量标志仍保留用于解释异常。MVP 0.3 的跌倒安全判定为相对开始姿态达到 65°并持续 400 ms，或三轴合加速度 ≤0.55 g / ≥1.60 g；任一路确认都会暂停训练并由控制任务锁存急停。65°高于 60°训练 ROM 硬上限，不与合法训练目标重叠。内嵌页面在手机 WebView 的本地存储中最多保留 100 次已结束会话，可导出 JSON/CSV；清除 App 数据会同时删除这些历史。

页面采用无重叠轮询：姿态/控制摘要 10 Hz、训练摘要 5 Hz、标定采集状态 1 Hz。ESP32-S3 使用双核配置，Wi-Fi/HTTP 任务固定在 Core 0，IMU 解析、稳定控制和推进器输出固定在 Core 1，避免网络请求挤占 10 ms 实时控制周期。
