...
训练状态响应包含 `algo`、`device_boot`、`last_score_pct`、`distance_cm_est` 和当前 `limits`，便于导出后追溯算法版本、区分重启后的会话 ID，并保留当次工程阈值。`qualified_pct` 在 MVP 0.2 中表示所有已完成步的连续轨迹评分均值；逐项二值质量标志仍保留用于解释异常。内嵌页面在手机 WebView 的本地存储中最多保留 100 次已结束会话，可导出 JSON/CSV；清除 App 数据会同时删除这些历史。
