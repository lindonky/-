...
可穿戴跌倒检测研究普遍结合加速度幅值、角速度、姿态改变和落地后稳定阶段；仅用一个冲击阈值更容易把日常活动误判为跌倒。一项多阶段状态机研究使用低加速度、随后冲击、稳定性和姿态改变逐级确认：[Wearable Fall Detection System with Real-Time Localization and Notification Capabilities](https://pmc.ncbi.nlm.nih.gov/articles/PMC12196599/)。另一项研究也显示“冲击后静止与姿态改变”比单加速度阈值减少日常活动误报：[Development of a Wearable-Sensor-Based Fall Detection System](https://pmc.ncbi.nlm.nih.gov/articles/PMC4346101/)。本项目的 JY61P 位于小腿中部，慢速受控模拟跌倒不会呈现典型陆地冲击，因此不能直接照搬腰部传感器的多条件阈值。

2026-08-13 四类实测样本显示：静止、正常抬腿、侧向偏移、慢速可控模拟跌倒的合加速度范围高度重叠，而模拟跌倒相对开始姿态的峰值倾角达到 73.5°，其他三类最高为 26.6°。因此 MVP 0.3 采用两条独立安全路径：

```text
角度路径：相对训练开始姿态的 pitch 或 roll 任一达到 65°，连续 400 ms
加速度路径：三轴合加速度 ≤0.55 g 或 ≥1.60 g，单样本立即确认
判定关系：角度路径 OR 加速度路径，任一路确认即判定疑似跌倒
安全动作：暂停会话 + 上报跌倒事件 + 闩锁急停 + 三路 PWM 回 1500 μs
恢复规则：只允许人工 START 解除急停，不自动恢复训练
```

65°高于当前 60°小腿 ROM 硬上限，不与允许的训练目标重叠；角度路径再设置 400 ms 防抖，以拒绝短暂大动作。加速度路径不要求同时出现高角速度，以覆盖自由落体或冲击，但会更重视安全、容忍较少的孤立毛刺。四组原始样本的统计、哈希和逐点回放结果见 [`CALIBRATION_ANALYSIS_20260813.md`](CALIBRATION_ANALYSIS_20260813.md)。研究阈值多来自腰部/躯干传感器和陆地模拟跌倒，当前数值仍只是小腿中部安装的工程起点，必须继续用快速抬腿、快速甩腿、治疗人员扶持、蹲下/坐下、绑带松动、水下失衡和受控模拟跌倒重标定。
