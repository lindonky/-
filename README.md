...
- 助力、主动、阻抗三种控制模式
- 闩锁式急停、IMU 超时保护、超角度保护，以及角度/加速度双路跌倒急停
...
- 训练中持续大角度或异常合加速度任一路成立都会暂停会话并锁存急停；解除急停后也不会自动恢复训练。
...
当前已有可编译的单腿主动训练 MVP 和本地报告链路，但尚未在真实设备、水下环境或人体上验证。当前主要后续工作是 JY61P 轴向确认、实机样本采集、动作/跌倒阈值标定，以及 T1/T3 纠偏方向与 T2 零助推的安全验证。
